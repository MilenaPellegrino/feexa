...
function calculateDatesExam() {
    ...
    if (dayExamen !== undefined) {
        ...
        startDates.forEach((fechaInicio, index) => {
            ...
            // Determinamos a qué grupo de exámenes pertenece según el mes
            const examMonth = examDate.getMonth() + 1; // 1 a 12
            let currentGroupTitle = "";
            if (examMonth >= 1 && examMonth <= 3) {
                currentGroupTitle = "<h3>Exámenes febrero</h3>";
            } else if (examMonth >= 6 && examMonth <= 8) {
                currentGroupTitle = "<h3>Exámenes julio/agosto</h3>";
            } else if (examMonth >= 11 || examMonth === 12) {
                currentGroupTitle = "<h3>Exámenes diciembre</h3>";
            } else {
                currentGroupTitle = "<h3>Exámenes especiales</h3>";
            }

            // Si cambiamos de grupo (ej. de julio/agosto a diciembre), reseteamos los contadores
            if (currentGroupTitle !== lastGroupTitle) {
                dateCounter = 1;
                lastGroupTitle = currentGroupTitle;
                groupHasDates = false;
            }
            ...
            const currentCall = dateCounter;
            dateCounter++;

            if (examDate >= today) {
                if (!groupHasDates) {
                    result.innerHTML += currentGroupTitle;
                    groupHasDates = true;
                }
                result.innerHTML += `<p><strong>Llamado ${currentCall}:</strong>  ${day}/${month}/${year}</p>`;
            }
        });

        if (result.innerHTML === "") {
            result.innerHTML = "<p>No hay más fechas de examen para esta materia en lo que queda del año.</p>";
        }
    } else {
